...
#+
from lsuv_eval import lsuv_eval #F1 F2
import errno
...
class lsuv(imdb): #F3
    ...
    def _get_lsuv_results_file_template(self): #F5
    ...
    def _write_lsuv_results_file(self, all_boxes): #F6
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print 'Writing {} VOC results file'.format(cls)
            filename = self._get_lsuv_results_file_template().format(cls)  #F7
    ...
    def _do_python_eval(self, output_dir = 'output'):
        ...
        for i, cls in enumerate(self._classes):
            if cls == '__background__':
                continue
            filename = self._get_lsuv_results_file_template().format(cls) #F8
            rec, prec, ap = lsuv_eval( #F9
    ...
    def evaluate_detections(self, all_boxes, output_dir):
        self._write_lsuv_results_file(all_boxes) #F10
        ...
        if self.config['cleanup']:
            for cls in self._classes:
                if cls == '__background__':
                    continue
                filename = self._get_lsuv_results_file_template().format(cls)#F11
                os.remove(filename)
